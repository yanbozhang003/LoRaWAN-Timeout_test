...
void LoRaWANNode::setCSMA(uint8_t backoffMax, uint8_t difsSlots, int enableCSMA) {
...
int16_t LoRaWANNode::uplink(uint8_t* data, size_t len, uint8_t port, bool confirm) {
  ...
  // Yanbo adds this line for debugging purpose
  this->phyLayer->setOutputPower(14);

  // Yanbo adds this line
  this->phyLayer->invertIQ(false);          // always use upchirp for uplink
...
int16_t LoRaWANNode::downlink(uint8_t* data, size_t* len) {
  ...
  // Yanbo adds this print
  // printf("Packet detected: %d || ACK set: %u ", packetDetected, (downlinkMsg[21]>>5)&1);
  // printf("|| Fcnt: %d\n", (uint16_t)downlinkMsg[23] << 8 | downlinkMsg[22]);

  // for (int i=16; i< downlinkMsgLen+16; i++){
  //   printf("%02x ", downlinkMsg[i]);
  // }
  // printf("\n");
  ...
  // get the frame counter and set it to the MIC calculation block             This may be the problem of MIC
...
bool LoRaWANNode::verifyMIC(uint8_t* msg, size_t len, uint8_t* key) {
  ...
  // printf("micCalculated: %d | micReceived: %d\n", micCalculated, micReceived);
...
uint8_t LoRaWANNode::findDataRate(uint8_t dr, DataRate_t* dataRate, const LoRaWANChannelSpan_t* span) {
  ...
  if(dataRateBand & RADIOLIB_LORAWAN_DATA_RATE_FSK_50_K) {
    ...
    dataRate->lora.spreadingFactor = 10;        // Yanbo fixed SF to 10
    dataRate->lora.codingRate = 5;              // Yanbo fixed coding rate to 5

    //printf("lora.codingRate: %u\n", dataRate->lora.codingRate);
  }

  //printf("DatarateFound: %u\n", dataRateFound);
...
int16_t LoRaWANNode::findChannelId(uint8_t dir, uint8_t* ch, uint8_t* dr, int8_t min, int8_t max) {
  ...
  int32_t chId = 1;       // Yanbo fixed channel to 1
  // printf("ChID: %d\n", chId);
...
void LoRaWANNode::performCSMA() {
    ...
    uint32_t BO = this->phyLayer->random(this->backoffMax / 2, this->backoffMax + 1);    // Yanbo modified lower bound to BO/2
    // printf("BO: %d\n", BO);
    ...
    while (BO > 0) {
        ...
        for (uint8_t i = 0; i < this->difsSlots; i++) {
            if (performCAD()) {
                ...
                // printf("OCCUPIED CHANNEL DURING DIFS\n");
                channelFreeDuringDIFS = false;
                // Channel is occupied during DIFS, hop to another.
                // this->setupChannels();       // Yanbo disabled frequency hopping for debugging purpose
                break;
            }
        }
        ...
        if (channelFreeDuringDIFS) {
            ...
            while (BO > 0) {
                if (performCAD()) {
                    ...
                    // printf("OCCUPIED CHANNEL DURING BO\n");
...
bool LoRaWANNode::performCAD() {
    ...
    // if(enableCSMA == 2){
    //     /*Yanbo test X-CSMA here*/
    //     // printf("count_temp: %d\n", count_temp);
    //     XCAD_CNT = XCAD_CNT+1;

    //     if ((XCAD_CNT % 4 == 1) || (XCAD_CNT % 4 == 2)){
    //       this->phyLayer->invertIQ(true);
    //     }else{
    //       this->phyLayer->invertIQ(false);
    //     }
    ...
    // }else if (enableCSMA == 1){
    //     this->phyLayer->invertIQ(false);
    // }

    if (enableCSMA == 9){
        this->phyLayer->invertIQ(false);
    }else if (enableCSMA != 0){
        XCAD_CNT = XCAD_CNT+1;

        int modular = enableCSMA * 2;
        int modular_rest = enableCSMA;

        // printf("XCAD_CNT: %d\n", XCAD_CNT);

        if ((XCAD_CNT % modular <= modular_rest) && (XCAD_CNT % modular != 0)){
          this->phyLayer->invertIQ(true);
        }else{
          this->phyLayer->invertIQ(false);
        }
      ...
      fprintf(stderr, "MAC uses ALOHA, should not approach here!\n");
      exit(1);
    }
